...
a quote-based span. The working file is the same quote-JSONL that `atos.gold compile`
consumes, so the flow is: annotate here -> `gold compile` -> `gold score`/eval.

    python -m atos.annotate --file gold/to_annotate.jsonl --port 8765 --host 0.0.0.0
...
from atos._annotate_ui import HTML as _HTML
...
def main(argv=None) -> int:
    ...
    p = argparse.ArgumentParser(prog="atos.annotate", description="Visual web annotator for the gold set.")
