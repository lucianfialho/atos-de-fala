"""Convert the Porttinari speech-act corpus (CSV) into atos JSONL.

The Porttinari corpus (da Silva et al., PROPOR 2024) is annotated at the
**sentence level** (one ISO 24617-2 communicative function per sentence). atos
...
mapped to the atos v1 act set (13 acts).

GRANULARITY CAVEAT: running ``atos.train.eval_cli`` (exact span-F1) against this
...
Label mapping reflects the atos v1 merges (answer/correction -> informar;
...
# Porttinari ISO label (normalized lowercase) -> atos v1 act
...
def map_label(porttinari_label: str) -> Optional[str]:
    """Map a Porttinari speech_act value to a atos act, or None if unmapped."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="atos.train.porttinari",
        description="Convert the Porttinari speech-act CSV into atos JSONL "
