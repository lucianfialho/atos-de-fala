---
type: entity
tags: [corpus, portuguese, speech-acts, dataset, holdout, atos-project]
sources: 1
updated: 2026-06-05
---
...
- **Real (human-annotated) PT-BR holdout** for atos's Plan-3 evaluation — satisfies the spec's
  "real-text holdout" requirement without us having to annotate one from scratch.
- Validates the atos taxonomy (same ISO basis; ~13 ML classes; labels map onto ours).
...
- Granularity: **sentence-level** (one act/sentence) — atos is span-level, so direct use is as
...
- Converter: `atos.train.porttinari` maps the CSV → atos JSONL (whole-sentence spans, labels
  mapped to our 13 acts; all 4091 rows map, 0 unmapped). Run:
  `python -m atos.train.porttinari --out data/porttinari-holdout.jsonl`. Eval against it reads as
